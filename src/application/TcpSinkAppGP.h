...
#ifndef __INET_TCPSINKAPPGP_H
#define __INET_TCPSINKAPPGP_H
 ...
 * on them.
 */
class INET_API TcpSinkAppGP : public TcpServerHostApp
{
  protected:
    long bytesRcvd = 0;
  ...
  public:
    TcpSinkAppGP();
    ~TcpSinkAppGP();
    friend class TcpSinkAppThreadGP;
};

class INET_API TcpSinkAppThreadGP : public TcpServerThreadBase
{
  protected:
    long bytesRcvd;
    TcpSinkAppGP *sinkAppModule = nullptr;
    simtime_t tStartAdded;
    simtime_t tEndAdded;
    ...
    virtual void init(TcpServerHostApp *hostmodule, TcpSocket *socket) override { TcpServerThreadBase::init(hostmodule, socket); sinkAppModule = check_and_cast<TcpSinkAppGP *>(hostmod);}
};
